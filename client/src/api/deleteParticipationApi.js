...
const handleError = (response, callback) => {
  if(isError(response)) {
    notification.error({
      ...
      description: 'Ocorreu um erro ao deletar a empresa no banco de dados. Se o erro persistir entre em contato com o desenvolvedor.'
    })
    callback(response)
    return true
  }
...
export const deleteParticipationApi = (id) => {
  return new Promise((onSuccess, onError) => {
    callServer('deleteParticipation', { urlParams: { id } })
      .then(response => {
