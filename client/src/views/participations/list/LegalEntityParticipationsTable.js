...
import {getParticipationsForLegalEntityApi} from '../../../api/getParticipationsApi'
...
const LegalEntityParticipationsTable = ({list, loading, goToShow, disableGoToShow, ControlCell}) => (
  <LoadingSpinnerWrapper loading={loading}>
    <Table>
      {list.map(item => (
        <TableRow key={item.id}>
          <TableLinkCell flex={2} onClick={() => !disableGoToShow && goToShow(item)}>
            <TitleSubtitle title={item.associatedEntity.name} subtitle={displayCode(item.legelEntity)} />
          </TableLinkCell>
          <TableCell flex={1}>
...
const mapActions = (dispatch, {legalEntityID}) => ({
  fetchData: () => {
    ...
    getParticipationsForLegalEntityApi(legalEntityID)
      .then(list => {
        dispatch(setParticipationsList(list, legalEntityID))
        dispatch(stopLoading(LOADING_KEY))
      })
      .catch(err => dispatch(stopLoading(LOADING_KEY)))
  },
  goToShow: participation => goToShowLegalEntity(dispatch, participation.associatedEntity),
})

const connectState = connect(mapState, mapActions)
const enhace = compose(connectState, willMount, showEmptyMessageIfHasNoData)

export default enhace(LegalEntityParticipationsTable)
