...
func ListIndividualEntities(db *sql.DB) ([]IndividualEntity, error) {
	q := "SELECT id, code, name, revisedAt, bornAt FROM Entity WHERE entityType = 'individual' ORDER BY id DESC LIMIT 20"
	rows, err := db.Query(q)
...
func ShowIndividualEntity(db *sql.DB, id int) (m IndividualEntity, err error) {
	m = IndividualEntity{}
	a := Address{}
	c := CountryQueryModel{}
	m.AfterInit()
	q := `
		...
		WHERE e.id = $1 AND e.entityType = 'individual'
	`
...
func CreateIndividualEntity(db *sql.DB, m *IndividualEntity) (validate.Validations, error) {
	q := `
		WITH a AS (
		...
		VALUES ($1, $2, $3, $4, 'individual', (SELECT id from a)) 
		RETURNING id
	`
	m.BeforeSave()
...
func UpdateIndividualEntity(db *sql.DB, id int, m *IndividualEntity) (validate.Validations, error) {
	m.BeforeSave()
	...
	q := `
		WITH e AS (
			UPDATE Entity SET %s
				WHERE id = $1 AND entityType = 'individual'
			RETURNING addressID
		) 
		UPDATE Address SET 
			id = id
